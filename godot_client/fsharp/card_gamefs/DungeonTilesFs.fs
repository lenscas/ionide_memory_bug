...
type DungeonTileState =
    | Draw of Poll<JsonData.DungeonLayout * (JsonData.TileAction -> int -> unit)>
    | Move of Poll<unit>
    | Nothing
...
type DungeonTilesFs() =
    ...
    override __._Process _ =
        match state with
        | Draw x -> x.Poll() |> ignore
        | Move x -> x.Poll() |> ignore
        | Nothing -> ()
